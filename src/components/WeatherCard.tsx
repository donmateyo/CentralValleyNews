...
export function WeatherCard({ locationKey }: WeatherCardProps) {
  ...
  return (
    <article className={`vp-card vp-weather vp-weather--${locationKey}`}>
      <header className="vp-weather__header">
        <div>
          <h3 className="vp-weather__title">{location.name}</h3>
          <p className="vp-meta">{location.county}</p>
        </div>
        {aqi && (
          <div className="vp-badge vp-badge--ghost" aria-label={`AQI ${aqi.value} ${aqi.label}`}>
            <span className="vp-meta">AQI</span>
            <span className={aqi.colorClass}>{aqi.value} {aqi.label}</span>
          </div>
        )}
      </header>

      <div className="vp-weather__main">
        {loading ? (
          <>
            <div className="vp-weather__icon vp-spin" aria-hidden="true">
              <svg className="vp-icon" fill="currentColor" viewBox="0 0 256 256">
            ...
            <div>
              <div className="vp-weather__temp">--°</div>
              <div className="vp-meta">Loading</div>
            </div>
          </>
        ) : error ? (
          ...
          <>
            <div className="vp-weather__icon">
              <WeatherIcon code={weather.weatherCode} className="vp-icon" />
            </div>
            <div>
              <div className="vp-weather__temp">{weather.temperature}°</div>
              <div className="vp-meta">{weather.description}</div>
            </div>
          </>
        ) : null}
      </div>

      <div className="vp-weather__footer">
        <div className="vp-weather__range">
          <span>H <strong>{weather?.high ?? '--'}°</strong></span>
          <span>L <strong>{weather?.low ?? '--'}°</strong></span>
        </div>
